...
def GlorotEmbedding(input_dim, output_dim, padding_idx=0):
    """Returns a Glorot initialized embedding layer for optimal gradient flow"""
    embedding = nn.Embedding(input_dim, output_dim, padding_idx=padding_idx)
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dim, char_embed_dim,
        ...
        # self.char_embedding_layer = GlorotEmbedding(input_dim, char_embed_dim, padding_idx=char_padding_idx)
        ...
        if fasttext_embeddings is not None:
            self.fasttext_embedding_layer = nn.Embedding.from_pretrained(fasttext_embeddings, padding_idx=word_padding_idx)
...
class Decoder(nn.Module):
    ...
    def __init__(self, input_dim, char_embed_dim,
        ...
        if embed_trg_gender:
            ...
            # self.gender_embedding_layer = GlorotEmbedding(gender_input_dim, gender_embed_dim)

        self.char_embedding_layer = nn.Embedding(input_dim, char_embed_dim, padding_idx=padding_idx)
        # self.char_embedding_layer = GlorotEmbedding(input_dim, char_embed_dim, padding_idx=padding_idx)
        # the input to the rnn is the context_vector + embedded token --> embed_dim + hidd_dim * 2
        # context_vector shape: [batch_size, hidd_dim * 2]
        self.rnn = nn.GRUCell((char_embed_dim + hidd_dim * 2), hidd_dim)
        # the input to the classifier is h_t + context_vector + embedded gender --> hidd_dim * 2 + hidd_dim + gender_embed_dim
        self.classification_layer = nn.Linear((hidd_dim * 2) + hidd_dim + gender_embed_dim, output_dim)
        # self.classification_layer = GlorotLinear((hidd_dim * 2) + hidd_dim + gender_embed_dim, output_dim)
...
class AdditiveAttention(nn.Module):
    ...
    def __init__(self, encoder_hidd_dim, decoder_hidd_dim):
        ...
        # self.v = GlorotLinear(decoder_hidd_dim, 1)
...
class Seq2Seq(nn.Module):
    ...
    def __init__(self, encoder_input_dim, encoder_embed_dim,
        ...
        self.linear_map = nn.Linear(encoder_hidd_dim * 2, encoder_hidd_dim)
        # self.linear_map = GlorotLinear(encoder_hidd_dim * 2, encoder_hidd_dim)

        self.decoder_hidd_dim = encoder_hidd_dim
    ...
    def forward(self, char_src_seqs, word_src_seqs, src_seqs_lengths, trg_seqs, trg_gender, teacher_forcing_prob=0.3):
        ...
        context_vectors = torch.zeros(batch_size, self.decoder_hidd_dim * 2)
